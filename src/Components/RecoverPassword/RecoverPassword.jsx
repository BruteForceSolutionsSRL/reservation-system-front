...
export default function RecoverPassword () {
    ...
    const validateCode = async () => {
        ...
        await fetch(url + "auth/change/password", {
            ...
            body: JSON.stringify({new_password: newPassword, confirmation_password: confirmPassword})
          }).then((response) => {
            ...
            console.error(err)
            responseFetch = {status: 500, data: { message: "Ocurrio un error inesperado, intentelo nuevamente." }}
        })
    ...
    const handleChangePassword = async () => {
        ...
        const {status, data} = await validateCode().finally(() => setLoadingChangePassword(false));
        ...
        <div>
            <div className="d-flex justify-content-center align-items-center" style={{ height:"100vh" }}>
                <div className="shadow p-3">
                    ...
                    <div>
                        <button className="btn btn-primary w-100 my-2" onClick={handleChangePassword} disabled={(!newPassword.trim() && !confirmPassword.trim()) && (newPassword !== "" && confirmPassword !== "")}>Cambiar contraseña</button>
                    </div>
                    <div>
                        {errMessage && (
